...
// create empty cityhistory array to store city name for local storage
var cityHistory = [];
...
function getWeatherToday() {
  ...
  fetch(getUrlCurrent)
    ...
    .then(function (response) {
      ...
      //Wind Speed
      var pElWind = $("<p>").text(`Wind Speed: ${response.wind.speed} M/S`);
      cardTodayBody.append(pElWind);
      //Set the lat and lon from the searched city
      var cityLon = response.coord.lon;
      // console.log(cityLon);
      var cityLat = response.coord.lat;
      // console.log(cityLat);

    //fetch api for uvi index
      ...
      fetch(getUrlUvi)
        ...
        .then(function (response) {
          ...
          //set the UV index with color warning
          if (uvi >= 0 && uvi <= 2) {
